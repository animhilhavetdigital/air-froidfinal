...
const INITIAL_USERS = [
  ...
  { id: 4, name: "Maroc Entreprise", email: "contact@marocentreprise.co.ma", role: "Client B2B", status: "Actif", lastConn: "Il y a 2 heures", scope: "Société B2B" },
  { id: 5, name: "Société Al Boustane", email: "y.boustane@alboustane.co.ma", role: "Client B2B", status: "En attente", lastConn: "Jamais connecté", scope: "Société B2B" },
];
...
export default function SuperAdminUsersPage() {
  ...
  const filteredUsers = users.filter(u => {
    ...
    const matchesRole = selectedRole === "Tous" || u.role.includes(selectedRole);
  ...
  return (
    <div ref={containerRef} className="p-6 md:p-10 max-w-7xl mx-auto flex flex-col gap-8">
      ...
      <div className="usr-item flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="font-nevan text-3xl md:text-4xl text-gray-900 uppercase tracking-wide mb-2">Gestion des Utilisateurs</h1>
          <p className="font-montserrat text-gray-500">Pilotez les comptes d'accès, définissez les rôles et gérez les permissions.</p>
        </div>
        <button 
      ...
      <div className="usr-item grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <div className="font-nevan text-2xl text-gray-900 mt-1">{users.filter(u => u.role !== "Client B2B").length}</div>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <div className="font-nevan text-2xl text-gray-900 mt-1">{users.filter(u => u.status === "Actif").length}</div>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <div className="font-nevan text-2xl text-gray-900 mt-1">{users.filter(u => u.status === "En attente").length}</div>
          </div>
        </div>
      </div>
      ...
      <div className="usr-item bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          >
            ...
            <option value="Client B2B">Clients B2B</option>
          </select>
        </div>
      </div>
          ...
          <table className="w-full text-left font-montserrat text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredUsers.length > 0 ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-gray-500 font-montserrat">
